...
export function getLayoutAgentInstructions(
  ...
  return [
    ...
    '- Apply changes via tools: apply_scoped_overrides, set_style, set_text, resize_grid_item, resize_box, set_element_override, remove_element_override, reorder_children.',
    '',
    'Tool rules (internal — never mention these mechanics to the user):',
    '- Only use element paths that appear in the snapshot.',
    '- Stay inside the current edit scope. If the user selected an element, edit only that selected element or its descendants.',
    '- For ambiguous phrases like "this", "this card", "the title", or "the button", prefer elements inside the selected subtree.',
    '- If the user asks for a page-wide change while the scope is selected-subtree, explain simply that they should select a larger container or the page area first.',
    ...
    '- Prefer apply_scoped_overrides for several related changes inside one selected card/panel.',
    '- Prefer resize_grid_item for grid cards with layout.isGridItem and maxColumnSpan; use resize_box for height or non-grid boxes.',
